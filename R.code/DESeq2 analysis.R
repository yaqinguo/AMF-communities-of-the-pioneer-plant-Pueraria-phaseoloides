...
data1$ID <- newcol
data1 <- data1 %>%
  mutate(Genus=str_replace(Genus,"^(\\S*)$","*\\1*"))
theme_set(theme_bw())
...
p <- ggplot(data1, aes(y=ID, x=log2FoldChange,color=Genus))+
  ...
  scale_color_manual(breaks=c("*Acaulospora*","*Rhizophagus*","*Paraglomus*",
                             "*Claroideoglomus*","*Dominikia*"),
                    values =c(brewer.pal(5,"Dark2")))+
...
p
ggsave("Deseq_results_markdown.tiff",width = 5,height = 4)
library(xlsx)
